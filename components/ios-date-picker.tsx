...
const IOSDatePicker = ({ onDateSelect, initialDate = new Date(), onClose }: IOSDatePickerProps) => {
  ...
  useEffect(() => {
    try {
      ...
      setFormattedDate(format(new Date(), "dd/MM/yyyy"));
    }
  }, [currentDate]);
  ...
  return (
    <div className="relative">
      ...
      {showDatePicker && (
        <>
          <div className="fixed inset-0 backdrop-standard backdrop-z-base" />
          <div
          ref={datePickerRef}
          className={`fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[330px] max-w-[95vw] glass-modal backdrop-z-modal rounded-md p-4 transition-all duration-150 ${animation}`}
          style={{
        ...
        >
          ...
          <div className="mt-2 flex justify-end space-x-2">
            <button
              ...
              className="px-2 py-1 text-xs font-medium text-gray-500 hover:text-gray-700 rounded-md hover:bg-gray-100 transition-colors duration-200"
            >
              Đóng
            </button>
          </div>
